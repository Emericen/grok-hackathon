...
TOOLS = [
    {"name": "bash", "description": "Run a shell command in the case directory.",
     "parameters": {"type": "object", "properties": {"command": {"type": "string"}}, "required": ["command"]}},
    {"name": "read_file", "description": "Read a case file (handles images, scanned PDFs, docx, xlsx).",
     "parameters": {"type": "object", "properties": {"path": {"type": "string"}}, "required": ["path"]}},
    {"name": "write_file", "description": "Write a file (creates parent dirs).",
     "parameters": {"type": "object", "properties": {"path": {"type": "string"}, "content": {"type": "string"}},
                    "required": ["path", "content"]}},
    {"name": "done", "description": "Declare the task complete.",
...
def exec_tool(run_dir, name, args):
    if name == "bash":
        r = subprocess.run(args["command"], shell=True, cwd=run_dir, capture_output=True, timeout=60)
...
def run_agent(src_fs, task_text, run_dir, provider, model, base_url, max_steps=50,
              system_prompt=SYSTEM_PROMPT, extra_mount=None):
...
def main():
    ...
    ap.add_argument("case", help="a twin world dir (has task.json) or any real case dir (needs --task)")
    ap.add_argument("--task", help="task text; required when the case dir has no task.json")
    ...
    existing = [int(p.name.split("-")[1]) for p in out_root.glob("run-*") if p.name.split("-")[1].isdigit()]
