...
**A data pipeline that turns real experts' AI-failure moments into anonymized, verifier-scored training worlds that frontier labs can legally train on.**
...
A US small-business tax engagement: an HVAC contractor's document pile (73 files) after an
IRS underreporting notice. Scans, phone photos of receipts, the client's own spreadsheets,
and an email whose claims contradict three of the documents.

A frontier model worked it and produced a polished five-part work product. An expert
reviewing that work caught two things it had asserted without checking:

- a $1,749 gap between sales tax collected and sales tax remitted, waved off as "timing"
  in the risk register while the same workbook's exception log still listed it as an open
  question;
- a client spreadsheet whose TOTAL cell held a stale `=SUM(D3:D42)` against data running
  to row 60. The model recovered the right annual figure by re-summing the column, then
  explained the discrepancy by asserting the cell was blank. It never opened the cell.

Those two corrections became verifiers. The twin built from them (identities swapped,
amounts frozen, packaging scan clean) was handed to the same model in a fresh session with
a plain professional ask and no hints. It missed both traps again.

That is the whole product in one run: an expert's ordinary review pass, converted into a
world that measurably discriminates, containing none of the original client's data.
